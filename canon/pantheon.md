# Pantheon & Religion

> The gods, spirits, and demons of Ymir, and how they are worshipped. Primary
> source: **Player Guide** pp. 21–24. Creation and the planes are in
> `cosmology.md`; the Sovereign Demons also touch `bestiary.md` and `history.md`.

## The nature of worship

`[CANON]` *(Player Guide p.21)* Ymir has a vast number of cults and creeds, but
most of the planet practices a kind of **monolatry**: no one doubts that the many
gods exist, but each person worships **one** in particular, seen as the true
representation of the Absolute that contains all the others. Few on Ymir dare
doubt the gods' power — clerics don't need to perform public miracles to prove
divine existence *(Player Guide p.10)*.

## The Prime Duality

| God | Title | Domains | Symbol |
|-----|-------|---------|--------|
| **Ech** | The One | The primordial unity; source of Spirits and magic; split to begin time | — |
| **Mithra** | The Creator, Lord of the Sun and Light | Justice, Good, light, life, creation, the Aether, peace, healing | Golden sun |
| **Ahriman** | The Corruptor, Supreme God of Evil | Injustice, Evil, darkness, destruction, death, the Nether, war, disease; bearer of **999 Plagues**; chief enemy of Mithra | Black sun |

## The Six Divine Twins (elemental gods)

`[CANON]` Born of the contrast between Mithra and Ahriman; each presides over one
element.

| God | Element | Title / domains | Symbol |
|-----|---------|-----------------|--------|
| **Diis** | Fire | The Lady of Flames; fire and passion, giver of life and destruction | Heart wreathed in flame |
| **Hudemia** | Water | The Just Mother of Waters; Law and justice | A spear holding the pans of a balance |
| **Varshnak** | Ice | The Beast; the dark side of nature, the fury of the elements, winter | An axe sprouting tree-roots from its haft |
| **Heis** | Air | The Mad God; wind, chance, fury, madness | A snarling monstrous face with a single horn |
| **Therìos** | Earth | The Hunter; forest and beasts, protector of travellers and hunters | A wolf's head |
| **Dingir** | Light | The Twofold; light and shadow, logic and Magic | A star |

## Other principal gods

`[CANON]` *(Player Guide pp.21–22)*

- **Ishtar** — the Mother Goddess; Moon, Love, and also War. Wife of Mithra.
  Symbol: silver moon.
- **Kur** — the Dark Lady; death and the underworld, giver of oracles and
  prophecy. Wife of Ahriman. Symbol: skull.
- **Thurms** — Lord of Thunder; war and storms, who leads the just to victory.
  Symbol: a thunderbolt.
- **Gog** — the Serpent; shadowy God of the Damned, of Vengeance, Lies, and
  Corruption. Ancient enemy of the good, and especially of Thurms. Symbol: a
  self-coiled serpent.
- **Aisen** — the Liberator; god of Freedom and Saviour of the Oppressed. Symbol:
  a broken chain.
- **Tifereth** — goddess of Love and Beauty. Symbol: a red rose.
- **Hokhmah** — goddess of Wisdom and Knowledge turned to Good. Symbol: a torch.
- **Tuchulcha** — the Darkener; god of Eternal Night and of Knowledge turned to
  Evil. Symbol: a raven.
- **Orum** — god of the Sea, protector of sailors and fishermen. Symbol: a shell.
  *(Chief cult of Cartosa — `factions.md`/`geography.md`.)*
- **Ranin** — the Merchant; transactions, artisans, commerce. Symbol: a hammer.
- **Diis / Heis / Hudemia / Therìos / Dingir / Varshnak** — the six elementals,
  listed in the table above.
- **Niethel** — the Source; a place (or state of existence) where the elves dwelt
  before coming to Ymir. Elven religion centres on this divine place. → `peoples.md`.

## The Xebechani pantheon

`[CANON]` *(Player Guide p.22)* The Xebechani keep a pantheon of their own; some
gods mirror the common ones, others are wholly distinct.

- **Quetzalcoatl** — the Feathered Serpent; Life, Birth, Good; "the Great
  Benefactor." *(= Mithra.)* Symbol: feathered serpent.
- **Omotecuthli** — Lord of Duality; Abundance and Wisdom. Symbol: a star.
- **Tezcatlipoca** — Lord of the Night Sky, protector of warriors (Lawful Good).
  Symbol: a jaguar.
- **Ehecatl** — the Inconstant *(= Heis)*. Symbol: chameleon.
- **Mictlantecuhtli** — Lord of the Underworld; Earth and beasts *(= Therìos)*.
  Symbol: a bat.
- **Xiuhtecuhtli** — Lord of the Land of Ash; Fire *(= Diis)*. Symbol: a baboon.
- **Tamoanchan** — the Incorruptible *(= Hudemia)*. Symbol: a gull.
- **Heichpyel** — the Observer; scribe of the gods, keeper of secrets that must
  not be revealed. Symbol: an owl.
- **Azatzotech** — the Great Putrefier; Putrefaction, Death, the Undead; "the
  Putrescent Horror" *(= Ahriman)*. Symbol: a condor.
- **Yogh** — the Spider-God; Poison and Assassination; "the Creeping Death."
  Symbol: a spider.
- **Xhulhu** — He Who Sleeps and Dreams; god of the Abysses, the Wrecker and
  Storm-Bringer; "the Madness from the Deep." Symbol: a tentacle.
  `[INFERRED]` A Lovecraftian/Cthulhu register distinct to the Xebechani —
  possibly linked to the outer god **Sguardo-sul-Nulla** ("Gaze-upon-the-Void")
  in the Cartosa timeline. → `history.md`, open question.

## The Outer Gods (Dèi Esterni / Antichi Dei) `[CANON — cosmologia.txt, dei-esterni.txt]`
> ⚠️ Links to the GM secret — see `secret-the-great-lie.md`. Openly, the Xebechani
> worship an "alien" pantheon; secretly these *are* the pre-Adepti **Ancient Gods**, the
> true sleeping deities the Adepti sealed away.
Six named Outer Gods (worshipping one caused "il Disastro"):
- **Tuulhak-Xanar** — the Observer.
- **Xuxhul'Xhathi** — the Judge; a "Trichotomy" of Good / Evil / Nothing.
- **Xatol-Nahuactl** — the Silent.
- **Tlahuiz'Halpantexuhtli** — the First.
- **Xotl-Nag'aaqe** — the Mother.
- **Xip-Tenalthe** — the Risen.
The wandering **Blood Moon**, **Sguardo-sul-Nulla** ("the Gaze into the Void"), is the
Observer stirring — the leading edge of the Ancient Gods' return in the Era della Marea.

## Racial creator-gods `[CANON — religione-nani.txt, etc.]`
- **Durkaam** (& his consort **Damkianna**) — the **Dwarves'** creator-god, said to have
  "made the world" and fathered the dwarves. `[CONFLICT with the secret: dwarves were
  engineered by the Omungus on Omalgat — see secret-the-great-lie.md]`
- **Niethel** — the elves' divine place of origin (→ `peoples.md`); the secret ties it to
  the world **Tchyo**.

## Regional & lesser deities `[CANON — regional docs]`
- **Thal'Khal** — a goddess worshipped in Helgedad's **Seamark** alongside **Hudemia**
  and **Diis** as a triad of feminine/maternal aspects (the **Torre Splendente**
  temple). *(Helgedad.doc.)*
- **Valrin** — the Dark Elves' **warrior god who leads the holy war**. *(Gaerwath.doc;*
  *note "Valrin" is also the Dark-Elf word for bound extraplanar "Forces" — a god and
  a class of demon-servant share the name.* `[CONFLICT?]` *→ NOTES.)*
- **Myethel** — the Dark Elves' **goddess of magic and oracles**. *(Gaerwath.doc; not
  to be confused with **Niethel**, the elves' divine place of origin.)*
- **The Wheel of Life (il Ciclo)** — the Sarradhi lizardfolk creed of the Sadhir;
  eternal reincarnation, blending spiritual and temporal rule. *(Sadhir gazetteer.)*
- The **Nkishi** *Orisha*/*Mayombe* forest-spirits and **M'Bantha** shamanism →
  `peoples.md`.

## Church politics `[CANON]`
The **Church of Mithra** is fracturing: Armora's Granduke declared **Solanthya's Alto
Teocrate** corrupt and founded the schismatic **Chiesa Armorita di Mithra**, accusing
the Letio Church of secretly worshipping **Ahriman**. A live religious war. → `factions.md`, `history.md`

## The Sovereign Demons

See `cosmology.md` (their nature, "Beyond the Circles") and the full ranked roster
in `history.md`/`bestiary.md`. In brief: the mightiest demons, near minor-deity in
power, most trusted vassals of Ahriman, whose sole aim is Ymir's destruction. **Ea**
(destroyed 1570 d.M.) and **Saxul** (destroyed 2563 d.M.) fell long ago; `[CANON — wiki,
updated]` by the Era della Marea their prison cracks and **most of the rest die too** —
the fullest roster marks all but **three** as *Defunto*, leaving only **Asmodeus**
(Tyranny), **Xardas** (the Puppeteer), and **Masak** (Illusions) to walk Ymir. Full
ranked roster and status in `bestiary.md`; the prison's cracking is in `history.md`.

## Known institutional religions (pointer)

- **Church of Mithra** — dominant state faith of the **Letio Empire**; assumed
  command over all Letia in 2610 d.M. Aggressively missionary. → `factions.md`.
- **Elemental cults** — notably around Meridian and among sailors. → `factions.md`.
- Race- and region-specific worship (Nkishi *Orisha*/*Mayombe* ancestor-spirits,
  M'Bantha shamanism, etc.) → `peoples.md`.

---
### Open questions
- The Player Guide lists ~20 "principal" gods but the cults are said to be
  countless — a fuller local-cults list may live in **Maraviglie**, **Edizione
  Straordinaria**, or the regional docs (not yet fully mined).
- Alignments are given for only some gods; the rest are `[INFERRED]` from domains.
- Is **Xhulhu / Sguardo-sul-Nulla** an "outer god" outside the Ech duality
  entirely? The Cartosa timeline treats Sguardo-sul-Nulla as an intruding
  **divinità esterna** — a possible third cosmological axis. → `[CONFLICT?]`
